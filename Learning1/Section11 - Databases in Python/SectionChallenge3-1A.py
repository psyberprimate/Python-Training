...
db.execute("CREATE TABLE IF NOT EXISTS transactions (time TIMESTAMP NOT NULL,"
           ...
           "offset INTEGER NOT NULL, PRIMARY KEY (time, account))")
...
class Account(object):
    ...
    def _save_update(self, amount): 
        ...
        deposit_time, zone = Account._current_time() # unpack tuple
        pickled_zone = pickle.dumps(zone)


        db.execute("UPDATE accounts SET balance = ? WHERE (name = ?)", (new_balance, self.name))
        db.execute("INSERT INTO transactions VALUES (?, ?, ?, ?)", (deposit_time, self.name, amount, pickled_zone))
        db.commit()
