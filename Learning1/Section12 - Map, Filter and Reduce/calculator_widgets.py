...
class CalculatorButton(tk.Button):
    """A button with callback, to be used in with a CalculatorGrid widget."""    
...
class CalculatorGrid(tk.Frame):
    """A tkinter Frame that display buttons and implements a simple calculator."""    
    ...
    #allowed_chars = [key[0] for key_row in keys for key in key_row] # only with tk.Entry
    
    def __init__(self, master, **kwargs):
        super().__init__(master, **kwargs)
        
        max_columns = max(len(row) for row in CalculatorGrid.keys)
        #self.result = tk.Entry(self) # only with tk.Entry
    ...
    def on_click(self, char: str):
        """Called by a button when it's clicked."""        
        if char == '=':
            # if self.result.get() and all(caption in CalculatorGrid.allowed_chars  # only with tk.Entry
            #                              for caption in self.result.get()):       # only with tk.Entry
            if self.result['text']:
                try:
                    #answer = str(eval(self.result.get())) # only with tk.Entry
                    ...
                    # self.result.delete(0, tk.END) # only with tk.Entry
                    # self.result.insert(0, answer) # only with tk.Entry
                    self.result['text'] = answer
        elif char == 'C':
            #self.result.delete(0, tk.END) # only with tk.Entry
            self.result['text'] = ''
        elif char == 'CE':
            #self.result.delete(len(self.result.get()) - 1, tk.END) # only with tk.Entry
            self.result['text'] = self.result['text'][:-1]
        else:
            #self.result.insert(tk.END, char) # only with tk.Entry
