...
def parse_invoice_number(invoice_number: str) -> tuple[int, int]:
    ...
    """
    #tuple_int = (int(invoice_number[0:4]), int(invoice_number[5:9])) #Slicing works too but split is more handy
    year, number = invoice_number.split("-")
    return int(year), int(number) #tuple_int #
...
def next_invoice_number(invoice_number: str) -> str:
    ...
    ##INITIAL SOLUTION ##
    # numbers = parse_invoice_number(invoice_number)
    ...
    # data = str(numbers[0])+"-"+str(format(numbers[1], '04') ) #"".join(str(numbers))
    # return data
...
def record_invoice(invoice_file: TextIO,
    ...
    :params last_line_ptr: The position of the start of the last line in the file. 
    This will be onbtained by the previous call to 'record_invoice'
...
data_file = 'Section8 - Reading and writing files/invoices.csv'
